...
function isAnagramInArray(anagram, arr){
    const sortedAnagram = anagram.replace(/\s/g, '').toLowerCase().split('').sort().join('');
    const sortedAnagramLength = sortedAnagram.length
    return arr.filter(str => {
        const sortedString = str.replace(/\s/g, '').toLowerCase();
        if (sortedAnagramLength !== sortedString.length){
            return false
        } else {
            return sortedAnagram === sortedString.split('').sort().join('')
        }
